...
class WindowAttention_D(nn.Module):
    ...
    def forward(self, x, rpi, mask=None):  # [16, 16*16, 180]
        ...
        ##########------(qk)*v+Linear------##########
        x = (attn @ v).transpose(1, 2).reshape(b_, n,
...
def window_partition(x, window_size):  # [1, 64, 64, 180]
    ...
    windows = x.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, window_size, window_size,
                                                            c)  # [1, 64//16, 64//16, 16, 16, 180]->#[16, 16, 16, 180]
    return windows
...
def window_partition_triangular(x, window_size, masks):  # [1, 64, 64, 180]
    ...
    windows = x.permute(0, 1, 3, 5, 2, 4).contiguous().view(-1, ws,
                                                            ws)  # b, h/ws, w/ws, c, ws, ws-->b*(h_ws)*(w_ws)*c, ws, ws
    ...
    window_masks = window_masks.view(m, -1, c, n).permute(1, 0, 3,
                                                          2).contiguous()  # [m, b*(h_ws)*(w_ws)*c, n]->[b*(h_ws)*(w_ws), m, n, c]
    return window_masks
...
class HAB_D(nn.Module):
    ...
    def forward(self, x, x_size, rpi_sa, attn_mask, triangular_masks):  # [1, 4096, 180], (64, 64)
        h, w = x_size
        b, _, c = x.shape
        ...
        if not self.triangular_flag:
            x_windows = window_partition(shifted_x,
                                         self.window_size)  # nw*b, window_size, window_size, c #[1, 64, 64, 180]->[16, 16, 16, 180]
        ...
        #######----W-MSA/SW-MSA----########
        attn_windows = self.attn(x_windows, rpi=rpi_sa, mask=attn_mask)  # [16, 256, 180]
        #######----W-MSA/SW-MSA----########
        ...
        if not self.triangular_flag:
            ...
            shifted_x = window_reverse(attn_windows, self.window_size, h,
                                       w)  # b h' w' c  #[16, 16, 16, 180]->[1, 64, 64, 180]
        #######----Merge_Windows----########
...
class HAB_S(nn.Module):
    ...
    def forward(self, x, x_size, rpi_sa, attn_mask, triangular_masks):  # [1, 4096, 180], (64, 64)
        ...
        x = F.pad(x,
                  (0, 0, pad_l, pad_r, pad_t, pad_b))  # [1, 64, 64, 180]-->[1, pad_t+64+pad_b, pad_l+64+pad_r, 0+180+0]
        ####----padding----####
        ...
        #######----W-MSA/SW-MSA----########
        attn_windows = self.attn(x_windows, rpi=rpi_sa, mask=attn_mask, sp_mask=attn_mask_sp)  # [16, 256, 180]
        #######----W-MSA/SW-MSA----########
        ...
        if not self.triangular_flag:
            ...
            shifted_sparse_x = window_reverse(attn_windows, self.window_size, Gh,
                                              Gw)  # b h' w' c  #[16, 16, 16, 180]->[1, 64, 64, 180]
        #######----Merge_Windows----########
...
class AttenBlocks(nn.Module):
    ...
    def __init__(self,
        ...
        ##HAB
        if not sparse_flag:
            self.blocks = nn.ModuleList([HAB_D(dim=dim,
            ...
            self.blocks = nn.ModuleList([HAB_S(dim=dim,
                                               input_resolution=input_resolution,
                                               num_heads=num_heads,
                                               window_size=window_size,
                                               shift_size=shift_size[i],
    ...
    def forward(self, x, x_size, params):  # [1, 4096, 180], (64, 64)
        ...
        x = self.overlap_attn(x, x_size, params['rpi_oca'])  # [1, 4096, 180]->[1, 4096, 180]
